...
def generate():
  ...
  for n in ncells:
    ...
    tempgeo = file("../../../src/transfinite_rectangle_template.geo", 'r')
    templines = tempgeo.readlines()
    tempgeo.close()
    
    lines = []
    for templine in templines:
      line = template(templine).substitute({'ncells':n, 'factor':0.01, 'aspect':2.5})
      lines.append(line)
...
def extract():
  ...
  table0.append("    & $\Nu$ & $v_{rms}$ & $q_1$ & $q_2$ & $q_3$ & $q_4$ & $T_{e_1}$ & $z_{e_1}$ & $T_{e_2}$ & $z_{e_2}$ \\\\\n")
  table0.append("\hline\n")
  ...
  for n in ncells:
    ...
    vtus = glob.glob(ioname+"0*.vtu")
    vtus = sorted(vtus, key=key)
    vtu = vtktools.vtu(vtus[-1])

    nu = -1.0*(stat["Stokes"]["Temperature"]["TopSurfaceIntegral"][-1])/2.5
    v_rms = sqrt(stat["Stokes"]["Velocity"]["L2NormSquared"][-1]/2.5)*Ra
    q1 = -det["TemperatureGradient"]["TemperatureGradient"]["TopLeft"][-1][-1]
    #q1 = -vtu.ProbeData(numpy.array([[0.0,1.0,0.0]]), "TemperatureGradient::TemperatureGradient")[-1][-1]
    #q2 = -det["TemperatureGradient"]["TemperatureGradient"]["TopRight"][-1][-1]
    q2 = -vtu.ProbeData(numpy.array([[2.5,1.0,0.0]]), "TemperatureGradient::TemperatureGradient")[-1][-1]
    #q3 = -det["TemperatureGradient"]["TemperatureGradient"]["BottomRight"][-1][-1]
    q3 = -vtu.ProbeData(numpy.array([[2.5,0.0,0.0]]), "TemperatureGradient::TemperatureGradient")[-1][-1]
    q4 = -det["TemperatureGradient"]["TemperatureGradient"]["BottomLeft"][-1][-1]
    #q4 = -vtu.ProbeData(numpy.array([[0.0,0.0,0.0]]), "TemperatureGradient::TemperatureGradient")[-1][-1]
    ...
    table0.append("{0:2d}$\\times${1:2d} & {2:.3f} & {3:.3f} & {4:.3f} & {5:.3f} & {6:.3f} & {7:.3f} & {8:.3f} & {9:.3f} & {10:.3f} & {11:.3f} \\\\\n".format(int(n*2.5), n, nu, v_rms, q1, q2, q3, q4, Te1, ze1, Te2, ze2))
  ...
  table0.append("\hline\n")
  table0.append("Benchmark & 6.930 & 171.755 & 18.484 & 0.177 & 14.168 & 0.618 & 0.397 & 0.191 & 0.576 & 0.784 \\\\\n")
...
def key(s):
  return int(s.split("0", 1)[-1].split(".vtu")[0])
